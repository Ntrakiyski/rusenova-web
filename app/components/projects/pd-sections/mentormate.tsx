...
export default function Mentormate({ projectData }: MentormateProps) {
  ...
  const introSection = projectData.sections.find(
    (section): section is IntroSection => section.type === "intro"
  );
  const projectSections = projectData.sections.filter(
    (section) => section.type !== "intro" && section.type !== "tech-stack"
  );
  ...
  return (
    <div className="min-h-screen w-full">
      ...
      <PDHero
        ...
        subtitle={
          (projectData as any).heroDescription || projectData.shortDescription
        }
        titleHighlight={(projectData as any).titleHighlight || ""}
        descriptionHighlight={
          (projectData as any).heroDescriptionHighlight || ""
        }
      ...
      {introSection && (
        <PDNutshell
          ...
          features={
            Array.isArray(introSection.content)
              ? introSection.content.map((item: any) => ({
